...
---------------- | -----------
`female_age`     | father's age in years.
`male_age`       | mother's age in years.
...
# Mother's age
hist(nc$male_age)
boxplot(nc$male_age, horizontal = TRUE)
summary(nc$male_age)
sd(nc$male_age)
IQR(nc$male_age)
...
```{r, eval = FALSE}
# Sample Data
xbar <- mean(nc$male_age)
std <- sd(nc$male_age)
samp_size <- length(nc$male_age)
...
t.test(x = nc$male_age, alternative = "two.sided", mu = 26.5, conf.level = 0.95)
```
...
keep_condition <- !is.na(nc$female_age)
f_age <- nc$female_age[keep_condition]
```
...
age_diff <- nc$male_age - nc$female_age  
summary(age_diff)
...
# setting paired = TRUE
t.test(x = nc$male_age, y = nc$female_age, alternative = "two.sided", mu = 0, paired = TRUE)
```
