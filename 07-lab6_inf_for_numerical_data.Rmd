---
title: 'Inference for Numerical Data (Lab 06)'
output:
  html_document:
    css: ../lab.css
    highlight: pygments
...
Please complete the **Inference for Numerical Data (Lab 06)** quiz on Canvas. The quiz questions mirror those provided throughout this lab. 
...
3.  Make a side-by-side box plot of `habit` and `weight`. What does the plot highlight about the relationship between these two variables?
...
4.  Check if the conditions necessary for inference are satisfied. Note that you will need to obtain sample sizes to check the conditions. You can compute the group size using the same `by` command above but replacing `mean` with `length`.

5.  Write the hypotheses for testing if the average weights of babies born to smoking and non-smoking mothers are different.
...
from a lab written by Mark Hansen of UCLA Statistics. The lab has been lightly edited for use in STAT 202 by Arend M Kuyper of the Department of Statistics at Northwestern University.
</div>